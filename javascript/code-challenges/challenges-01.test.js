...
const addExclamation = (arr) => {
  const exclaimArr = [];
  arr.forEach(inputStringA => {
    exclaimArr.push(inputStringA + '!');
  });
  return exclaimArr;
};
...
const allUpperCase = (arr) => {
  const upperArr = [];
  arr.forEach(inputStringB => {
    upperArr.push(inputStringB.toUpperCase());
  });
  return upperArr;
};
...
const greeting = (word) => {
  const greetingWord = word.toUpperCase() + '!';
  return greetingWord;
};
...
const speaker = (words, callback) => {
  const userGreeting = [];
  words.forEach(word => {
    userGreeting.push(callback(word));
  });
  return userGreeting;
};
